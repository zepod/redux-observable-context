...
describe('getData', () => {
    ...
    test('should THROW, if opener is MISSING', (done) => {
        ...
        try {
            singleContext.awaitData('MISSING').subscribe(observer)
        } catch {
    ...
    test('should not EMIT data, if context was never OPENED', () => {
        const observer = jest.fn()
        singleContext.awaitData('OPEN1').subscribe(observer)
        expect(observer).not.toBeCalled()
    })
    ...
    test('should EMIT data, if context is REOPENED', () => {
        const observer = jest.fn()
    ...
    test('should EMIT data from extended context', () => {
        ...
        const action = { type: 'OPEN1', payload: 'hello' };
        action$.next(action)
        multipleContext.awaitData('OPEN1').subscribe(observer)
        expect(observer).toBeCalledWith(action)
    })
})
